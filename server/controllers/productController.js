...
const getProducts = (req, res) => {
  client.query('SELECT * FROM products', (err, result) => res.status(200).json({
    message: 'success',
    result: result.rows,
  }));
};
...
const modifyProduct = (req, res) => {
  const { productId } = req.params;
  const {
    product_name, product_description, price, product_quantity, available_quantity,
  } = req.body;
...
export default {
  getProducts, modifyProduct, deleteProduct, getAProduct,
};
